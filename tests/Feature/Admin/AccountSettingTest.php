...
class AccountSettingTest extends TestCase
{
    ...
    {
        $response = $this->actingAs($this->tuUser())->get(route('admin.setting.account.edit'));
        $response->assertOk();
    }
    ...
    {
        $response = $this->actingAs($this->tuUser())->patch(route('admin.setting.account.update'), $this->userDataWithPassword());
        $response->assertOk();
    ...
    /** @test */
    public function admin_can_perform_update_without_password()
    {
        ...
        $old_password = User::find(1)->password;

        $response = $this->actingAs($this->tuUser())->patch(route('admin.setting.account.update'), $data_without_password);
        $response->assertOk();
